package com.charlton.mapeditor;

import com.charlton.mapeditor.components.MapEditorMenuBar;
import com.charlton.mapeditor.model.EditorMode;
import com.charlton.mapeditor.model.MapEditorModel;
import com.charlton.mapeditor.model.TileMapModel;
import com.charlton.mapeditor.model.TileSet;
import com.charlton.mapeditor.views.MapEditorTileSetView;
import com.charlton.mapeditor.views.MapEditorView;
...
public class MapEditorController {
    ...
    public void loadTileMap(String mapFile) {
        try (FileInputStream fis = new FileInputStream(mapFile); ObjectInputStream is = new ObjectInputStream(fis)) {
            model.mapModel = (TileMapModel) is.readObject();
            initializeViews();
            loadedFile = mapFile;
            mapNeedsSaving = true;
    ...
    private void initializeViews() {
        model.tileSet = new TileSet(model.mapModel.perTileWidth, model.mapModel.perTileHeight, model.mapModel.tileSetFile);
        mapEditorView.loadInitialMapView(model.mapModel);
    ...
    public void createTileMap(String file, int perTileWidth, int perTileHeight, int mapRows, int mapColumns) {
        model.mapModel = new TileMapModel(file, perTileWidth, perTileHeight, mapRows, mapColumns);
        initializeViews();
        loadedFile = null;
        mapNeedsSaving = true;
    }
    ...
    public void generateTileMap(String tileSetImageFile, int tileWidth, int tileHeight) {
        try {
            ...
            for (int row = 0; row < model.mapModel.mapRows; row++) {
                for (int col = 0; col < model.mapModel.mapColumns; col++) {
                    int nextTileIndex = (row * getTileSet().getTileSetColumns()) + col;
                    model.mapModel.mapLayout[row][col] = nextTileIndex;
                }
            }
            mapEditorView.loadInitialMapView(model.mapModel);
        } catch (IOException e) {
    ...
    public void updateTileInMap(int row, int col, boolean setEmpty) {
        if (!setEmpty)
            model.mapModel.mapLayout[row][col] = getTileSet().getTileImageList().indexOf(selectedTile);
        else model.mapModel.mapLayout[row][col] = -1;
    }
    ...
    public void updateCollisionTileInMap(int row, int col, boolean status) {
        model.mapModel.collisionMap[row][col] = status;
    }

    public void updateTileInObjectMap(int row, int col, boolean setEmpty) {
        if (!setEmpty)
            model.mapModel.objectMap[row][col] = getTileSet().getTileImageList().indexOf(selectedTile);
        else model.mapModel.objectMap[row][col] = -1;
    }
    ...
    public void addRowToLayout(String location) {
        int[][] currentMapLayout = model.mapModel.mapLayout;
        boolean[][] currentCollisionMap = model.mapModel.collisionMap;
        int[][] currentObjectMap = model.mapModel.objectMap;
        int newRowCount = model.mapModel.mapRows + 1;
        int colCount = model.mapModel.mapColumns;
        ...
        for (int row = location.equals("top") ? 1 : 0; row < model.mapModel.mapRows + offset; row++) {
            for (int col = 0; col < model.mapModel.mapColumns; col++) {
        ...
        model.mapModel.mapRows = newRowCount;
        model.mapModel.mapLayout = newMapLayout;
        model.mapModel.collisionMap = newCollisionMap;
        model.mapModel.objectMap = newObjectMap;
        mapEditorView.loadInitialMapView(model.mapModel);
    }
    ...
    public void setLastClickedRow(int clickedY) {
        this.lastClickedRow = clickedY / model.mapModel.perTileHeight;
    }

    public void setLastClickedCol(int clickedX) {
        this.lastClickedCol = clickedX / model.mapModel.perTileWidth;
    }
    ...
    public int getTileWidth() {
        return model.mapModel.perTileWidth;
    }

    public int getTileHeight() {
        return model.mapModel.perTileHeight;
    }
    ...
    public int getTileAt(int row, int col) {
        return this.model.mapModel.mapLayout[row][col];
    }

    public int getObjectTileAt(int row, int col) {
        return this.model.mapModel.objectMap[row][col];
    }

    public boolean getCollisionStatusAt(int row, int col) {
        return model.mapModel.collisionMap[row][col];
    }
    ...
    public BufferedImage getTileImage(int row, int col) {
        return getTileSet().getTileImageList().get(model.mapModel.mapLayout[row][col]);
    }

    public BufferedImage getObjectTileImage(int row, int col) {
        return getTileSet().getTileImageList().get(model.mapModel.objectMap[row][col]);
    }
