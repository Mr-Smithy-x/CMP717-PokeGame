package com.charlton.mapeditor.views;

import com.charlton.mapeditor.MapEditorController;
...
public class MapEditorView extends JPanel {
    ...
    public void loadInitialMapView(TileMapModel mapModel) {
        int mapWidth = mapModel.perTileWidth * mapModel.mapColumns;
        int mapHeight = mapModel.perTileHeight * mapModel.mapRows;
        ...
        for (int row = 0; row < mapModel.mapRows; row++) {
            for (int col = 0; col < mapModel.mapColumns; col++) {
                if (mapModel.mapLayout[row][col] != -1) {
                    mapViewGraphics.drawImage(editorController.getTileImage(row, col),
                            col * mapModel.perTileWidth, row * mapModel.perTileHeight, null);
                ...
                drawCollisionBorder(mapModel.collisionMap[row][col], mapViewGraphics, row, col,
                        mapModel.perTileWidth, mapModel.perTileWidth);
            }
        }
