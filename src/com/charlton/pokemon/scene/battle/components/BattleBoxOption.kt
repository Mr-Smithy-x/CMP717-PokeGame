...
class BattleBoxOption : Renderable {
    ...
    val options = arrayOf(
        arrayOf(Option(BattleOption.FIGHT, true), Option(BattleOption.BAG, false)),
        arrayOf(Option(BattleOption.POKEMON, false), Option(BattleOption.RUN, false))
    )
    ...
    override fun render(g: Graphics) {
        ...
        for(i in options.indices) {
            for(j in options.indices) {
                drawString(g, i, j, options[i][j].text.name)
            }
        }
    ...
    private fun drawSelection(g: Graphics) {
        ...
        if (option.selected) {
            ...
            val strWidth = g.fontMetrics.stringWidth(option.text.toString())
    ...
    fun getSelection(): Option {
        return options[rowSelection][columnSelection]
    }


    data class Option(val text: BattleOption, var selected: Boolean = false)
