...
class MyPokemonHudComponent(var pokemon: Pokemon) : Renderable {
    ...
    override fun render(g: Graphics) {
        ...
        val containerHeight = height / 5
        val containerWidth = (width / 2.5).toInt()



        val heightOfBox = height / 3
        val containerStartOfY = height - heightOfBox - (containerHeight / 0.8).toInt()


        val containerStartOfX = (width * 0.55).toInt()
        ...
        val base_percentage = (1 - ((pokemon.stats.maxHealth.toFloat() - pokemon.health.toFloat()) / pokemon.stats.maxHealth.toFloat()))
        ...
        g.fillRoundRect(
            ...
            (containerHeight * (1 - .33)).toInt() + containerStartOfY,
            lifeWidth + x / 2 + 1,
            15,
            10,
            10
        ...
        g.fillRoundRect(lifeBarStart - 1, (containerHeight * (1 - .31)).toInt() + containerStartOfY, lifeWidth, 13, 10, 10)

        g.color = Color.decode("0x666666")
        g.drawRoundRect(lifeBarStart, (containerHeight * (1 - .30)).toInt() + containerStartOfY, lifeBarEnd, 10, 10, 10)
        //endregion
        ...
        g.fillRoundRect(
            /* x = */  lifeBarStart,
            /* y = */ (containerHeight * (1 - .30)).toInt()  + containerStartOfY,
        ...
        g.drawString(
            ...
            (containerStartOfY * (1 - .12)).toInt() + g.fontMetrics.height / 2 + containerHeight
        )

        g.color = Color.BLACK
        g.font = FontHelper.get("font.ttf", 20)
        g.drawString(pokemon.name, (containerStartOfX + (containerWidth * 0.05)).toInt(), containerStartOfY+g.fontMetrics.height)

        g.color = Color.BLACK
        fontWidth = g.fontMetrics.stringWidth("Lv${pokemon.level}")
        g.drawString("Lv${pokemon.level}", ((containerStartOfX + lifeBarEnd) + lifeBarEnd - fontWidth * 2 + fontWidth / 3), containerStartOfY+g.fontMetrics.height)
        /*
